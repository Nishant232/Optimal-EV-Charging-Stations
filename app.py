...
def run_optimization():
    ...
    def optimization():
        ...
        import time
        import GOA
        test_status['logs'].append('\nStarting Grasshopper optimization algorithm...')
        for i in range(0, 20, 5):
            test_status['progress'] = i
            test_status['logs'].append(f'Grasshopper Optimization Algorithm progress: {i}%')
            time.sleep(0.2)
        
        import particle_swarm
        test_status['logs'].append('\nStarting Particle Swarn Optimization...')
        for i in range(20, 40, 5):
            test_status['progress'] = i
            test_status['logs'].append(f'Particle Swarn Optimization progress: {i}%')
            time.sleep(0.2)
        ...
        for i in range(40, 60, 5):
            test_status['progress'] = i
            test_status['logs'].append(f'Genetic Algo progress: {i}%')
            time.sleep(0.2)
        
        import evsc_optimization
        test_status['logs'].append('\nStarting Optimization with Market Features...')
        for i in range(60, 80, 5):
            test_status['progress'] = i
            test_status['logs'].append(f'EVCS Optimization progress: {i}%')
            time.sleep(0.2)

        # Simulate Distance step
        import virtual_distance
        test_status['current_step'] = 2
        test_status['logs'].append('\nStarting distance matrix generation...')
        ...
        for i in range(90, 100, 5):
            test_status['progress'] = i
            test_status['logs'].append(f'Gray Wolf Optimization progress: {i}%')
            time.sleep(0.2)
